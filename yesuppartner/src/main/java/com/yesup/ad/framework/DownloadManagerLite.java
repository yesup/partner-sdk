                ...
                if (request != null) {
                    ...
                    try {
                        ...
                        if (connection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                            ...
                            Log.v(TAG, "Download completed:"+request.getDownloadUrl());
                        } else {
                            request.onRequestCompleted(YesupHttpRequest.STATUS_CONNECT_FAILED);
                            Log.v(TAG, "Connect failed[HTTP-"+connection.getResponseCode()+"]:"+request.getDownloadUrl());
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                        request.onRequestCompleted(YesupHttpRequest.STATUS_FAILED);
                        Log.v(TAG, "Download failed:"+request.getDownloadUrl());
                    } finally {
                        try{
                            if (output != null) {
                                output.close();
